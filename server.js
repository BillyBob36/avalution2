...
app.post('/api/realtime-voice', async (req, res) => {
    try {
        const { audio, voice, conversationHistory } = req.body;
        ...
        ws.on('message', (data) => {
            try {
                ...
                if (event.type === 'error') {
                    console.error('Realtime Voice API error:', event.error);
                    clearTimeout(timeout);
                    ws.close();
                    if (!res.headersSent) {
                        res.status(500).json({ error: event.error?.message || 'Realtime Voice API error' });
                    }
                }
            } catch (e) {
                console.error('Error parsing realtime voice message:', e);
            }
        });
